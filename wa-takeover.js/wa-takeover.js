...
export default async function handler(req, res) {
  ...
  if (!contact_id || typeof bot_activo !== "boolean")
    return res.status(400).json({ error: "Faltan contact_id o bot_activo" });
  try {
    ...
    const { error } = await supabase.from("whatsapp_contacts")
      .update(update).eq("id", contact_id);
    if (error) throw error;
    res.status(200).json({ ok: true, bot_activo });
  } catch (err) {
    console.error("wa-takeover error:", err);
